...
from better_enum.utils import _is_descriptor, _is_dunder, _is_sunder, _reduce_ex_by_name
...
class TestEnum(TestCase):
	...
	def test_programatic_function_from_unicode_dict(self):
		SummerMonth = Enum(
				'SummerMonth', dict(((unicode('june'), 1), (unicode('july'), 2), (unicode('august'), 3)))
				)
		lst = list(SummerMonth)
		self.assertEqual(len(lst), len(SummerMonth))
		self.assertEqual(len(SummerMonth), 3, SummerMonth)
